...
Read this before any testing on this project. (Formerly the project's `AGENTS.md`; the
`§2`–`§4` references in tests and `docs/QA-CHECKLIST.md` mean the sections below.)
...
- `docs/COMPLIANCE.md` — the legal contract the site must satisfy
- `docs/large_scale_luxury_rugs_mansion_guide.md` — positioning (the client's own guide; where it conflicts with `docs/COMPLIANCE.md`, compliance wins)
- `docs/QA-CHECKLIST.md` — testable assertions, each citing its source

A spec written by the agent under test is not a spec. That failure has already happened
once on this project: an earlier session wrote its own spec and declared it authoritative.
...
that no testimonial or rating markup exists anywhere (`docs/COMPLIANCE.md`), and that a form
submission reaches `/enquire/success`.

Screenshots count as evidence; `page.screenshot()` into the session scratchpad (not the
repo), then look at it.
...
Web3Forms. Intercept in the browser so no real Drive uploads,
...
The form reads the reply and only shows success on `{ ok: true }`. The header matches the
real backend, but note that `fulfill()` does not enforce CORS — a stub without it is still
readable, so a CORS failure can only be tested against a real server.
...
`class="hidden" aria-hidden="true"` wrapper. `enquire.astro`'s submit handler bounces any submission that
...
URL before the next backend-touching change, rather than skipping this rung. Still open.)
